...
describe 'plugin post process' do
  before do
    SiteSetting.discourse_in_post_topic_list_enabled = true
  ...
  context 'creating a post with markup' do
    ...
    context 'using tags attribute markup' do
      it 'processes it' do
        ...
        post = create_post(raw: "[topics tags=#{tag_1.name},#{tag_2.name} order=created]\n[/topics]")
        post.rebake!
        post.reload
    ...
    context 'using exceptTopicIds attribute markup' do
      it 'processes it' do
      topic = create_topic
      post = create_post(topic: topic, raw: "[topics exceptTopicIds=#{topic.id}]\n[/topics]")
      post.rebake!
      post.reload
  ...
  context 'rebaking all posts with topic query' do
    it 'rebakes the post' do
      post = create_post(raw: "[topics order=\"created\"]\n[/topics]")
